...
class GCSConnector:
    ...
    def __init__(self):
        try:
            ...
            self.bucket_name = ONEDRIVE_SERVER_CONFIG.landing_zone_bucket
    ...
    def _grant_user_access(self, app_name: str, user_id: str) -> None:
        ...
        """
        try:
            # Grant access at the app_name/user_id/ level, not session level
            resource_prefix = (
            ...
            # Determine member string (if user_id is an email vs something else, but typically user:email)
            ...
            iam_policy.bindings.append(
                {
                    ...
                    "members": {member},
                    "condition": {
                        "title": "mcp-landing-zone-access",
                        "description": f"Auto-generated by OneDrive MCP Server for user {user_id}",
            ...
            logger.info(
                f"Granted roles/storage.objectViewer to '{member}' for prefix '{resource_prefix}'"
            )
