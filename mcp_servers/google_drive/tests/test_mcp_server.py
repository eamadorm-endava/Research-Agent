...
async def test_list_files_success(mock_drive_manager):
    ...
    manager.list_files.return_value = [
        MagicMock(to_dict=lambda: {"id": "1", "name": "Doc", "mimeType": "application/pdf"})
    ]
...
async def test_get_file_text_success(mock_drive_manager):
    manager = MagicMock()
    manager.get_file_text.return_value = MagicMock(
        to_dict=lambda: {
            "id": "f1",
            "name": "Notes",
            "mimeType": "text/plain",
...
async def test_create_google_doc_success(mock_drive_manager):
    manager = MagicMock()
    manager.create_google_doc_from_text.return_value = MagicMock(
        to_dict=lambda: {"id": "doc1", "name": "Summary", "mimeType": "application/vnd.google-apps.document"}
    )
    mock_drive_manager.return_value = manager
...
async def test_upload_pdf_success(mock_drive_manager):
    manager = MagicMock()
    manager.upload_pdf_from_text.return_value = MagicMock(
        to_dict=lambda: {"id": "pdf1", "name": "Summary.pdf", "mimeType": "application/pdf"}
    )
    mock_drive_manager.return_value = manager
