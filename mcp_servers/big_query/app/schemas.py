...
# Reusable Types
PROJECT_ID = Annotated[AvailableProject, Field(description="The mandatory GCP Project ID.")]
DATASET_ID = Annotated[str, Field(description="The ID of the dataset.", pattern=r"^[\w-]+$", min_length=1, max_length=1024)]
TABLE_ID = Annotated[str, Field(description="The ID of the table.", pattern=r"^[\w-]+$", min_length=1, max_length=1024)]
LOCATION = Annotated[str, Field(default="US", description="The geographic location.")]
QUERY = Annotated[str, Field(description="The standard SQL query to execute.", min_length=1)]
# Type for schema structures (definitions)
SCHEMA_DEFINITION = Annotated[
    List[Dict[str, Any]], 
    Field(description="A list of dictionaries defining the schema, e.g., [{'name': 'id', 'type': 'INTEGER'}].")
]
# Type for data rows (both input for insertion and output for query results)
ROWS = Annotated[
    List[Dict[str, Any]], 
    Field(description="A list of dictionaries representing data rows, where each dict is a mapping of column names to values.")
]
...
class BaseResponse(BaseModel):
    ...
    """
    execution_status: Annotated[Literal["success", "error"], Field(description="The status of the execution.")]
    execution_message: Annotated[str, Field(default="Execution completed successfully.", description="Detailed message about the execution or error description.")]
    ...
    def check_read_only(cls, v: str) -> str:
        ...
        for keyword in destructive_keywords:
            if re.search(rf"\b{keyword}\b", v, re.IGNORECASE):
                raise ValueError(f"This tool only support read-only queries. Destructive command '{keyword}' detected.")
        return v
