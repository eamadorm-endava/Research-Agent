...
description: Expert protocol for high-fidelity data retrieval and analysis across EKB, Google Drive, Google Calendar, BigQuery, Google Cloud Storage, and Microsoft SharePoint using Contextual Anchoring and Parallel Discovery.
---
...
- "List document libraries for the SharePoint site named Finance"
- "Find the SharePoint file called Architecture Overview"
- "What budget was approved for project X?"
→ Converge fast. If the user names SharePoint, start with the SharePoint Search Protocol; otherwise use the two-wave EKB search + GCS long-context escalation while running Drive, Calendar, and SharePoint expansion in parallel.
...
→ Cast a wide net across all sources. Synthesize across EKB, Calendar, BQ, Drive, GCS, and SharePoint.
...
- Both return zero → skip Wave 2 and GCS Long Context. Proceed immediately to Calendar Search Protocol + Drive Search Protocol + SharePoint Search Protocol in parallel. **Keyword source for Drive and SharePoint**: extract entities (companies, projects, technologies, people) directly from the user's original prompt — do not wait for EKB results. These user-prompt keywords are the primary input for Drive (Stage 0 entity extraction) and SharePoint site/drive-item searches. Do not use EKB-derived terms as input because there are none.
...
Run the **CALENDAR SEARCH PROTOCOL**, **DRIVE SEARCH PROTOCOL**, and **SHAREPOINT SEARCH PROTOCOL** (from the system prompt) in parallel with Wave 2, starting all four streams immediately after Wave 1 completes. Calendar, Drive, and SharePoint run regardless of Wave 1 outcome — they are never skipped, even when EKB returned results. **Keyword priority for Drive and SharePoint**: first use terms extracted from the user's original prompt (company names, project names, technologies, people); then supplement with any entities derived from Wave 1 EKB results.
...
For SharePoint specifically: search sites first using the highest-confidence entity keywords; for relevant sites, list document libraries; then search drive items using the same project, company, technology, or document-title keywords. Do not claim SharePoint is unavailable unless the SharePoint tool returns an authorization or configuration error.
...
2. **Anchor Extraction**: Build a "Context Graph" from the merged results of both calls:
   ...
   - **Relational Mapping**: map project names to their associated companies and tech stacks — use these as primary anchors for Phase 2 Drive, Calendar, and SharePoint searches.
...
- Re-run Drive and SharePoint searches with full keyword decomposition if the entity map differs meaningfully from keywords already used. Do not repeat identical `list_files`, `search_sharepoint_sites`, or `search_sharepoint_drive_items` calls.
- Do NOT repeat `ekb_semantic_search`, `ekb_keyword_search`, `get_current_time`, `list_calendar_events`, or SharePoint calls already made.

**Discovery → Targeted Fallback:**
If Discovery Mode has exhausted all phases (Phase 1 through Level 5) without finding the answer, run Targeted Mode's unique steps — skipping any tool calls already made:
...
- Do NOT repeat Wave 1, calendar calls, Drive calls, SharePoint calls, or BQ calls already made.
...
|:---:|:---:|:---:|:---:|:---:|
| EKB / Drive / Cloud Storage / BigQuery / SharePoint | Project name or `Unknown` | Human-readable file or event name | Author email or display name | `YYYY-MM-DD` |
...
- **Drive and SharePoint entries**: only cite actual files or document-library items used for the answer — never include folders or broad site/library discovery results as references unless the user specifically asked to list sites, libraries, or folders.
