...
async def get_gcp_secret(secret_id: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code != 200:
            raise ValueError(f"Failed to fetch secret {secret_id}: {resp.status_code} {resp.text}")
        data = resp.json()
...
async def main() -> None:
    ...
    for key in ["JIRA_USER_EMAIL", "JIRA_API_TOKEN", "JIRA_INSTANCE_URL", "JIRA_CLOUD_ID"]:
    ...
    server_proc = subprocess.Popen(
        [
            "uv", "run", "--group", "mcp_atlassian",
            "python", "-m", "mcp_servers.atlassian.app.main",
            "--host", "127.0.0.1", "--port", "8085",
            "--transport", "streamable-http", "--log-level", "WARNING"
        ],
        env=server_env,
    ...
    if server_proc.poll() is not None:
        ...
        print(f"   [ERROR] MCP Server failed to start. Code: {server_proc.returncode}", file=sys.stderr)
    ...
    try:
        ...
        if not atlassian_toolset:
            print("   [ERROR] Atlassian MCP toolset is not mounted on the research_agent!", file=sys.stderr)
            sys.exit(1)
        
        print(f"   Found toolset pointing to: {atlassian_toolset._connection_params.url}")
        ...
        print("   " + "-" * 60)
        print(f"   Discovery breakdown: {jira_count} Jira tools, {confluence_count} Confluence tools.")

        # Verification asserts
        assert len(discovered_tools) == 18, f"Expected 18 tools, but discovered {len(discovered_tools)}"
        assert jira_count == 6, f"Expected 6 Jira tools, but got {jira_count}"
        assert confluence_count == 12, f"Expected 12 Confluence tools, but got {confluence_count}"
