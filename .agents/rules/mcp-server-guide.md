...
- **Security Constraints (MANDATORY)**:
  ...
  - **Dynamic Authorization**: After the file is ingested using the MCP Service Account, the MCP server **must** automatically grant the user read access to their specific namespace folder in the Landing Zone by injecting an IAM condition (`resource.name.startsWith(...)`) into the bucket's IAM policy.
