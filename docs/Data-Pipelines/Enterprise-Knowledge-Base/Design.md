...
**Domain Buckets:**
- `gs://<project_id>-kb-it/`
- `gs://<project_id>-kb-finance/`
- `gs://<project_id>-kb-hr/`
- `gs://<project_id>-kb-sales/`
- `gs://<project_id>-kb-executives/`
- `gs://<project_id>-kb-legal/`
- `gs://<project_id>-kb-operations/`
...
/{project_name}/
  {tier}/
    {uploader_email_prefix}/
      {filename}
```
*Example: `gs://<project_id>-kb-it/project-alpha/client-confidential/maria.gutierrez/architecture.pdf`*
...
|---|---|---|
| `document_id` | `STRING` (REQUIRED) | Deterministic UUID (Shared across versions) |
| `gcs_uri` | `STRING` (REQUIRED) | Final routed path in domain bucket |
| `filename` | `STRING` (REQUIRED) | Original filename |
| `classification_tier` | `STRING` (REQUIRED) | String Label (confidential, public, etc.) |
| `domain` | `STRING` (REQUIRED) | it, hr, sales, etc. |
| `confidence_score` | `FLOAT64` (REQUIRED) | AI classifier confidence (0.0 - 1.0) |
| `trust_level` | `STRING` (REQUIRED) | Published, WIP, Archived |
| `project_id` | `STRING` (REQUIRED) | Matches `project_name` |
| `uploader_email` | `STRING` (REQUIRED) | Account that uploaded the file |
| `description` | `STRING` (REQUIRED) | AI Summary (Generated via Gemini) |
| `version` | `INT64` (REQUIRED) | Incremental version number |
| `latest` | `BOOL` (REQUIRED) | Flag for the current active version |
| `ingested_at` | `TIMESTAMP` (REQUIRED) | ISO 8601 timestamp |
...
| `document_id` | `STRING` (REQUIRED) | Foreign Key to metadata |
