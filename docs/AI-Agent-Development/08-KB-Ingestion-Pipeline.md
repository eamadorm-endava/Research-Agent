...
- `GCS_KB_INGESTION_BUCKET`: (Usually `<project_id>-kb-landing-zone`) where documents are moved before ingestion.

## Workflow
1. User requests ingestion.
2. Skill collects metadata and validates against BigQuery.
3. Skill moves the file from `<project_id>-ai-agent-landing-zone` to `<project_id>-kb-landing-zone`.
