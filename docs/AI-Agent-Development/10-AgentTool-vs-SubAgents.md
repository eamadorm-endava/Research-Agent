...
sequenceDiagram
    ...
    Spec->>Spec: Zero-copy MultimodalFileInjectionPlugin injects file
...
The Research Specialist fetches files via MCP servers that trigger the `MultimodalFileInjectionPlugin`. This plugin produces `file_data` parts that must appear in the final user-visible response. With `AgentTool`, these parts are stripped by the text-only extraction step. With `sub_agents=`, they flow through the shared session to the user.
...
- **[07-Storage-and-Ingestion-Architecture.md](07-Storage-and-Ingestion-Architecture.md)**: Details on the zero-copy external data ingestion pipeline.
