...
1. **Delegated Token Flow**: Gemini Enterprise secures user consent (via OAuth 2.0 flow) and issues a delegated access token. The Agent retrieves this from its context (via `GEMINI_GOOGLE_AUTH_ID`) using the `get_ge_oauth_token` helper and forwards it downstream via the standard `Authorization` header.
