...
class TestGCSManager(unittest.TestCase):
    ...
    def test_create_bucket_success(self):
        ...
        self.mock_client_instance.create_bucket.assert_called_with("test-bucket", location="US")

    def test_create_bucket_failure(self):
        self.mock_client_instance.create_bucket.side_effect = GoogleCloudError("Creation failed")
    ...
    def test_upload_object_string_content(self):
        ...
        result = self.gcs_manager.create_object("test-bucket", "test.txt", content="hello")
        
        self.assertEqual(result.name, "test.txt")
        mock_blob.upload_from_string.assert_called_with("hello", content_type="text/plain")
    ...
    def test_upload_object_bytes_content(self):
        ...
        result = self.gcs_manager.create_object("test-bucket", "data.bin", content=b"\x00\x01")
        
        self.assertEqual(result.name, "data.bin")
        mock_blob.upload_from_string.assert_called_with(b"\x00\x01", content_type="application/octet-stream")
    ...
    def test_upload_object_local_path(self, mock_guess, mock_exists):
        ...
        result = self.gcs_manager.create_object("test-bucket", "remote.png", local_path="/tmp/local.png")
        
        self.assertEqual(result.name, "remote.png")
        mock_blob.upload_from_filename.assert_called_with("/tmp/local.png", content_type="image/png")
    ...
    def test_update_object_metadata(self):
        ...
        result = self.gcs_manager.update_object_metadata(
            "test-bucket", 
            "doc.txt", 
    ...
    def test_list_blobs(self):
        ...
        self.mock_client_instance.list_blobs.assert_called_with(mock_bucket, prefix="data/")
