...
case "$COMMAND" in
    delete-agent)
        if [ -z "$APP_ID" ] || [ -z "$DISPLAY_NAME" ]; then
            echo "Error: --app-id and --display-name are required for delete-agent."
            exit 1
        fi
        echo "Looking for agent with display name: $DISPLAY_NAME..."
        ...
        AGENT_ID=$(echo "$HTTP_BODY" | jq -r ".agents[]? | select(.displayName == \"${DISPLAY_NAME}\") | .name" | head -n 1)
        ...
        if [ -n "$AGENT_ID" ] && [ "$AGENT_ID" != "null" ]; then
            ...
            echo "No existing agent found with name ${DISPLAY_NAME}."
        fi
        ;;

    delete-auth-id)
        if [ -z "$AUTH_ID" ]; then
            echo "Error: --auth-id is required for delete-auth-id."
            exit 1
        fi
        AUTH_ID_URL="${BASE_URL}/authorizations/${AUTH_ID}"
        ...
        echo "Delete command completed for Auth ID."
        ;;
    ...
    create-auth-id)
        ...
        if [ "$CREATE_CODE" -ne 200 ] && [ "$CREATE_CODE" -ne 201 ] && [ "$CREATE_CODE" -ne 202 ]; then
             echo "Failed to create Auth ID. HTTP Status: $CREATE_CODE. Response: $(echo "$CREATE_RESPONSE" | sed '$d')"
             exit 1
        fi
        echo "Auth ID creation requested successfully."
        ;;

    register-agent)
        if [ -z "$APP_ID" ] || [ -z "$DISPLAY_NAME" ] || [ -z "$ADK_RESOURCE_ID" ] || [ -z "$AUTH_ID" ] || [ -z "$AGENT_ENGINE_LOCATION" ]; then
        ...
        REASONING_ENGINE_PATH="projects/${PROJECT_ID}/locations/${AGENT_ENGINE_LOCATION}/reasoningEngines/${ADK_RESOURCE_ID}"
        
        echo "Registering Agent ${DISPLAY_NAME} in GE..."
        
        JSON_PAYLOAD=$(jq -n \
            --arg displayName "$DISPLAY_NAME" \
            --arg adkResourceId "$REASONING_ENGINE_PATH" \
            --arg authId "projects/${PROJECT_NUMBER}/locations/${GE_LOCATION}/authorizations/${AUTH_ID}" \
    ...
    *)
        echo "Usage: $0 {delete-agent|delete-auth-id|create-auth-id|register-agent} [flags]"
        exit 1
        ;;
esac
